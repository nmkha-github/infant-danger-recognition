...
epoch = int(
    open(
        os.path.join(
            project_path,
            "saved_models/VisionTransformerNoPretrainedModel/current_epoch.txt",
        )
    )
    .read()
    .strip()
...
if epoch > 0:
    ...
    model.load_state_dict(
        torch.load(
            os.path.join(
                project_path,
                f"saved_models/VisionTransformerNoPretrainedModel/epoch_{previous_epoch}/VisionTransformerNoPretrainedModel_epoch_{previous_epoch}.pth",
            )
        )
    )
    optimizer.load_state_dict(
        torch.load(
            os.path.join(
                project_path,
                f"saved_models/VisionTransformerNoPretrainedModel/epoch_{previous_epoch}/optimizer_epoch_{previous_epoch}.pth",
            )
        )
    )
...
while True:
    ...
    if epoch % 4 == 0:
        save_dir = os.path.join(
            project_path,
            f"saved_models/VisionTransformerNoPretrainedModel/epoch_{epoch}/",
        )
        ...
        torch.save(
            ...
            os.path.join(
                project_path,
                f"saved_models/VisionTransformerNoPretrainedModel/epoch_{epoch}/VisionTransformerNoPretrainedModel_epoch_{epoch}.pth",
            ),
        )
        torch.save(
            ...
            os.path.join(
                project_path,
                f"saved_models/VisionTransformerNoPretrainedModel/epoch_{epoch}/optimizer_epoch_{epoch}.pth",
            ),
        )
        ...
        with open(
            os.path.join(
                project_path,
                f"saved_models/VisionTransformerNoPretrainedModel/epoch_{epoch}/loss.txt",
            ),
            "w",
        ) as file:
        ...
        with open(
            os.path.join(
                project_path,
                f"saved_models/VisionTransformerNoPretrainedModel/epoch_{epoch}/accuracy.txt",
            ),
            "w",
        ) as file:
