...
epoch = int(
    open(os.path.join(project_path, "saved_models/NoDropoutModel/current_epoch.txt"))
    .read()
    .strip()
)
...
if epoch > 0:
    ...
    model.load_state_dict(
        torch.load(
            os.path.join(
                project_path,
                f"saved_models/NoDropoutModel/epoch_{previous_epoch}/SimpleModel_epoch_{previous_epoch}.pth",
            )
        )
    )
...
while True:
    ...
    if epoch % 1 == 0:
        ...
        torch.save(
            ...
            os.path.join(
                project_path,
                f"saved_models/NoDropoutModel/epoch_{epoch}/SimpleModel_epoch_{epoch}.pth",
            ),
        )
        torch.save(
