...
epoch = int(
    open(os.path.join(project_path, "saved_models/GCNDropoutModel/current_epoch.txt"))
    .read()
    .strip()
)
...
if epoch > 0:
    ...
    model.load_state_dict(
        torch.load(
            os.path.join(
                project_path,
                f"saved_models/GCNDropoutModel/epoch_{previous_epoch}/GCNDropoutModel_epoch_{previous_epoch}.pth",
            )
        )
    )
    optimizer.load_state_dict(
        torch.load(
            os.path.join(
                project_path,
                f"saved_models/GCNDropoutModel/epoch_{previous_epoch}/optimizer_epoch_{previous_epoch}.pth",
            )
        )
    )
...
while True:
    ...
    if epoch % 1 == 0:
        save_dir = os.path.join(
            project_path, f"saved_models/GCNDropoutModel/epoch_{epoch}/"
        )
        ...
        torch.save(
            ...
            os.path.join(
                project_path,
                f"saved_models/GCNDropoutModel/epoch_{epoch}/GCNDropoutModel_epoch_{epoch}.pth",
            ),
        )
        torch.save(
            ...
            os.path.join(
                project_path,
                f"saved_models/GCNDropoutModel/epoch_{epoch}/optimizer_epoch_{epoch}.pth",
            ),
        )

        with open(
            os.path.join(
                project_path, f"saved_models/GCNDropoutModel/epoch_{epoch}/loss.txt"
            ),
            "w",
        ) as file:
        ...
        with open(
            os.path.join(
                project_path,
                f"saved_models/GCNDropoutModel/epoch_{epoch}/accuracy.txt",
            ),
            "w",
        ) as file:
        ...
        with open(
            os.path.join(
                project_path, "saved_models/GCNDropoutModel/current_epoch.txt"
            ),
            "w",
        ) as file:
