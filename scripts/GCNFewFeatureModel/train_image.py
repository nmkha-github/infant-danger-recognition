...
from models.GCNFewFeatureModel.GCNFewFeatureModel import GCNFewFeatureModel
...
num_class = 56
model = GCNFewFeatureModel(num_class)
...
epoch = int(
    open(
        os.path.join(project_path, "saved_models/GCNFewFeatureModel/current_epoch.txt")
    )
    .read()
    .strip()
)
...
if epoch > 0:
    ...
    model.load_state_dict(
        torch.load(
            os.path.join(
                project_path,
                f"saved_models/GCNFewFeatureModel/epoch_{previous_epoch}/GCNFewFeatureModel_epoch_{previous_epoch}.pth",
            )
        )
    )
    optimizer.load_state_dict(
        torch.load(
            os.path.join(
                project_path,
                f"saved_models/GCNFewFeatureModel/epoch_{previous_epoch}/optimizer_epoch_{previous_epoch}.pth",
            )
        )
    )
...
while True:
    ...
    if epoch % 10 == 0:
        save_dir = os.path.join(
            project_path, f"saved_models/GCNFewFeatureModel/epoch_{epoch}/"
        )
        ...
        torch.save(
            ...
            os.path.join(
                project_path,
                f"saved_models/GCNFewFeatureModel/epoch_{epoch}/GCNFewFeatureModel_epoch_{epoch}.pth",
            ),
        )
        torch.save(
            ...
            os.path.join(
                project_path,
                f"saved_models/GCNFewFeatureModel/epoch_{epoch}/optimizer_epoch_{epoch}.pth",
            ),
        )

        with open(
            os.path.join(
                project_path, f"saved_models/GCNFewFeatureModel/epoch_{epoch}/loss.txt"
            ),
            "w",
        ) as file:
        ...
        with open(
            os.path.join(
                project_path,
                f"saved_models/GCNFewFeatureModel/epoch_{epoch}/accuracy.txt",
            ),
            "w",
        ) as file:
        ...
        with open(
            os.path.join(
                project_path, "saved_models/GCNFewFeatureModel/current_epoch.txt"
            ),
            "w",
        ) as file:
